...
public class GetInvoices {
    ...
    public ArrayList<MasterSales> masterSalesArrayList;
    public ArrayList<TransactionSales> transactionSalesArrayList;
    public ArrayList<Payment> paymentArrayList;
    private GetInvoices.CallBack listener;
    String businessID, OutletId, OutletTypeID, RoleID;
    public String resultMessage = "";
    public String errorCode = "GSI";
    public String apiResponse = "";

    public GetInvoices(String businessID, String OutletId, String RoleID, String OutletTypeID, GetInvoices.CallBack listener){
        this.businessID = businessID;
        this.OutletId = OutletId;
        this.RoleID = RoleID;
        this.OutletTypeID = OutletTypeID;
        this.listener = listener;
        new GetInvoices.apitask().execute();
    }

    public interface CallBack {
        void afterGetInvoices();
    }
    ...
    public class apitask extends AsyncTask<Object, Object, Void> {
        ...
        protected void onPreExecute() {
            ...
            masterSalesArrayList = new ArrayList<>();
            transactionSalesArrayList = new ArrayList<>();
            paymentArrayList = new ArrayList<>();

            if(RoleID.equals("6") && OutletTypeID.equals("1")) {
                url = Uri.parse(URL.GSTGetInvoicesURL).buildUpon()
...
//                Log.d("check", jsonObject+"");
                ...
                if (jsonObject != null) {
                    ...
                    JSONArray jsonArray = dataJsonObject.getJSONArray("Invoices");
                    for (int i = 0; i < jsonArray.length(); i++) {
                        MasterSales masterSales;
                        Gson gson = new Gson();
                        String jsonString = jsonArray.getJSONObject(i).toString();
                        masterSales = gson.fromJson(jsonString, MasterSales.class);

                        // sales date
                        masterSales.setSynced(DatabaseHandler.M_SALES_SYNCED_CODE_Synced);
                        masterSalesArrayList.add(masterSales);
                    }
                    jsonArray = dataJsonObject.getJSONArray("TransactionSales");
                    for (int i = 0; i < jsonArray.length(); i++) {
                        TransactionSales transactionSales;
                        Gson gson = new Gson();
                        String jsonString = jsonArray.getJSONObject(i).toString();
                        transactionSales = gson.fromJson(jsonString, TransactionSales.class);
                        transactionSalesArrayList.add(transactionSales);
                    }
                    jsonArray = dataJsonObject.getJSONArray("Payments");
                    for (int i = 0; i < jsonArray.length(); i++) {
                        Payment payment;
                        Gson gson = new Gson();
                        String jsonString = jsonArray.getJSONObject(i).toString();
                        payment = gson.fromJson(jsonString, Payment.class);
                        payment.setSynced(DatabaseHandler.PAYMENT_SYNCED_CODE_Synced);
