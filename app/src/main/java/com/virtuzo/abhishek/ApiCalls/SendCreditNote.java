...
public class SendCreditNote {
    ...
    private SendCreditNote.CallBack listener;
    public String errorCode = "S CN";
    String businessID;
    String jsonString;
    ...
    public SendCreditNote(String businessID, String TabCode, SendCreditNote.CallBack listener) {
        this.TabCode = TabCode;
        this.businessID = businessID;
        this.listener = listener;
        this.jsonString = isApiCall();
        new SendCreditNote.apitask().execute();
    }

    private String isApiCall() {
        creditNoteArrayList = DatabaseHandler.getInstance().getUnsyncedCreditNotes();

        // optimization
        if(creditNoteArrayList.size() == 0) {
    ...
    public class apitask extends AsyncTask<Object, Object, String> {
        ...
        protected String doInBackground(Object... params) {
            ...
            try {
                for(CreditDebitNote creditNote : creditNoteArrayList) {
                    jsonString = createOneJSONObject(creditNote);
                    ...
                    if(message.equals(MyFunctions.SUCCESS_MESSAGE)) {
                        DatabaseHandler.getInstance().singleCreditNoteSyncSuccessful(creditNote.getCreditNoteID());
                    } else {
                        break;
                    }
        ...
        protected void onPostExecute(String message) {
            ...
            listener.afterSendCreditNote();
        }

    }

    public String createOneJSONObject(CreditDebitNote creditNote) throws JSONException {
...
//                masterSalesReturn.setOutletAddress(address);
        Gson gson = new Gson();
        String jsonString = gson.toJson(creditNote);
        JSONObject jsonMasterObject = new JSONObject(jsonString);

        Log.i("CreditDebitNote", jsonMasterObject.toString());
        jsonArray.put(jsonMasterObject);

        Log.i("CreditDebitNote", jsonArray.toString());
        exportJson.put("CreditDebitNote", jsonArray);
    ...
    public JSONObject sendDataToServer(String url, String jsonString) {
        ...
        try {
            ...
            url = Uri.parse(URL.GSTSendCreditNotesURL).buildUpon()
                    .appendQueryParameter("BusinessId", businessID)
                    .appendQueryParameter("CreditDebitNote", jsonString).toString();

            urlHit = Uri.parse(URL.GSTSendCreditNotesURL).buildUpon()
