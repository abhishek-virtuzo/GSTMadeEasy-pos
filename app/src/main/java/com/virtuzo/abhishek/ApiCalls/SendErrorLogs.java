...
public class SendErrorLogs {

    private String TabCode;
    private SendErrorLogs.CallBack listener;
    public String errorCode = "S EL";
    String businessID;
    String jsonString;
    ...
    public SendErrorLogs(String businessID, String TabCode, SendErrorLogs.CallBack listener) {
        this.TabCode = TabCode;
        this.businessID = businessID;
        this.listener = listener;
        this.jsonString = isApiCall();
        new SendErrorLogs.apitask().execute();
    }

    private String isApiCall() {
        errorLogArrayList = DatabaseHandler.getInstance().getUnsyncedErrorLogList();

        // optimization
        if(errorLogArrayList.size() == 0) {
    ...
    public class apitask extends AsyncTask<Object, Object, String> {
        ...
        protected String doInBackground(Object... params) {
            ...
            String message = "";
            try {
                for(ErrorLog errorLog : errorLogArrayList) {
                    jsonString = createOneJSONObject(errorLog);
                    ...
                    if(message.equals(MyFunctions.SUCCESS_MESSAGE)) {
                        DatabaseHandler.getInstance().singleErrorLogSyncSuccessful(errorLog.getID());
                    }
        ...
        protected void onPostExecute(String message) {
            ...
            listener.afterSendErrorLogs();
        }

    }

    public String createOneJSONObject(ErrorLog errorLog) throws JSONException {
        JSONObject exportJson = new JSONObject();
        JSONArray jsonArray = new JSONArray();

        Gson gson = new Gson();
        String jsonString = gson.toJson(errorLog);
        JSONObject jsonMasterObject = new JSONObject(jsonString);

        Log.i("Logs", jsonMasterObject.toString());
        jsonArray.put(jsonMasterObject);

        Log.i("Logs", jsonArray.toString());
        exportJson.put("Logs", jsonArray);
    ...
    public JSONObject sendDataToServer(String url, String jsonString){
        ...
        try {
            ...
            url = Uri.parse(URL.GSTSendLogsURL).buildUpon()
                    .appendQueryParameter("BusinessId", businessID)
                    .appendQueryParameter("Logs", jsonString).toString();
