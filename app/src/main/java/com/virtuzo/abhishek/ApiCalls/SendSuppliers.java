...
public class SendSuppliers {

    private String TabCode;
    private SendSuppliers.CallBack listener;
    public String errorCode = "SSP";
    String businessID;
    String jsonString;
    ...
    public SendSuppliers(String businessID, String TabCode, SendSuppliers.CallBack listener) {
        this.TabCode = TabCode;
        this.businessID = businessID;
        this.listener = listener;
        this.jsonString = isApiCall();
        new SendSuppliers.apitask().execute();
    }

    private String isApiCall() {
        supplierArrayList = DatabaseHandler.getInstance().getUnsyncedSuppliersListFromDB();
    ...
    private String createOneJSONObject(Supplier supplier) {
        try {
            ...
            Log.i("Supplier List", jsonArray.toString());
            exportJson.put("SupplierList", jsonArray);
        ...
        protected String doInBackground(Object... params) {
            ...
            try {
                for (Supplier supplier : supplierArrayList) {
                    jsonString = createOneJSONObject(supplier);
                    ...
                    if(message.equals(MyFunctions.SUCCESS_MESSAGE)) {
                        DatabaseHandler.getInstance().singleSupplierSyncSuccessful(supplier.getSupplierId());
                    } else {
                        break;
                    }
                }
                Log.i("Send Supplier Message", message);
            } catch (Exception e) {
                e.printStackTrace();
            }
            return message;
    ...
    public JSONObject sendDataToServer(String url, String jsonString){
        ...
        try {
            ...
            url = Uri.parse(URL.GSTSendSuppliersURL).buildUpon()
                    .appendQueryParameter("BusinessID", businessID)
                    .appendQueryParameter("Suppliers", jsonString).toString();

            urlHit = Uri.parse(URL.GSTSendSuppliersURL).buildUpon()
