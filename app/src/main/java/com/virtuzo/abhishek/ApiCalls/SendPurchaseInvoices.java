...
public class SendPurchaseInvoices {

    private String TabCode;
    private SendPurchaseInvoices.CallBack listener;
    public String errorCode = "SPI";
    String businessID;
    String jsonString;
    ArrayList<MasterPurchase> masterPurchaseArrayList;
    public String resultMessage = "";
    public String apiResponse = "";
    public String urlHit = "";

    public SendPurchaseInvoices(String businessID, String TabCode, SendPurchaseInvoices.CallBack listener) {
        this.TabCode = TabCode;
        this.businessID = businessID;
        this.listener = listener;
        this.jsonString = isApiCall();
        new SendPurchaseInvoices.apitask().execute();
    }

    private String isApiCall() {
        masterPurchaseArrayList = DatabaseHandler.getInstance().getUnsyncedPurchaseListFromDB();

        // optimization
        if(masterPurchaseArrayList.size() == 0) {
    ...
    public interface CallBack {
        void afterSendPurchaseInvoices();
    }
    ...
    public class apitask extends AsyncTask<Object, Object, String> {
        ...
        protected String doInBackground(Object... params) {
            ...
            try {
                for(MasterPurchase masterPurchase : masterPurchaseArrayList) {
                    jsonString = createOneJSONObject(masterPurchase);
                    ...
                    if(message.equals(MyFunctions.SUCCESS_MESSAGE)) {
                        DatabaseHandler.getInstance().singlePurchaseSyncSuccessful(masterPurchase.getPurchaseID()+"");
                    } else {
                        break;
                    }
        ...
        protected void onPostExecute(String message) {
            ...
            listener.afterSendPurchaseInvoices();
        }

    }

    public String createOneJSONObject(MasterPurchase masterPurchase) throws JSONException {
        JSONObject exportJson = new JSONObject();
        JSONArray jsonArray = new JSONArray();
        // change address format
//                String address = masterPurchase.getOutletAddress().replace("###", ",");
//                masterPurchase.setOutletAddress(address);
        Gson gson = new Gson();
        String jsonString = gson.toJson(masterPurchase);
        JSONObject jsonMasterObject = new JSONObject(jsonString);

        ArrayList<TransactionPurchase> transactionPurchaseArrayList =
                DatabaseHandler.getInstance().getPurchaseListDetailsFromDB(masterPurchase.getPurchaseID()+"");
        JSONArray jsonTransactionArray = new JSONArray();
        for(TransactionPurchase transactionPurchase : transactionPurchaseArrayList) {
            Gson gson1 = new Gson();
            String jsonString1 = gson1.toJson(transactionPurchase);
            JSONObject jsonTransactionObject = new JSONObject(jsonString1);
            jsonTransactionArray.put(jsonTransactionObject);
        }
        jsonMasterObject.put("TransactionPurchase", jsonTransactionArray);

        Log.i("Purchases", jsonMasterObject.toString());
        jsonArray.put(jsonMasterObject);

        Log.i("Purchases", jsonArray.toString());
        exportJson.put("Purchases", jsonArray);
    ...
    public JSONObject sendDataToServer(String url, String jsonString){
        ...
        try {
            ...
            url = Uri.parse(URL.GSTSendPurchasesURL).buildUpon()
                    .appendQueryParameter("BusinessId", businessID)
                    .appendQueryParameter("Purchases", jsonString).toString();

            urlHit = Uri.parse(URL.GSTSendPurchasesURL).buildUpon()
