...
public class SendEwayBill {

    private String TabCode;
    private SendEwayBill.CallBack listener;
    public String errorCode = "S EWB";
    String businessID;
    String jsonString;
    public String resultMessage = "";
    public String apiResponse = "";
    public String urlHit = "";
    ArrayList<EwayBill> ewayBillArrayList;

    public SendEwayBill(String businessID, String TabCode, SendEwayBill.CallBack listener) {
        this.TabCode = TabCode;
        this.businessID = businessID;
        this.listener = listener;
        this.jsonString = isApiCall();
        new SendEwayBill.apitask().execute();
    }

    private String isApiCall() {
        ewayBillArrayList = DatabaseHandler.getInstance().getUnsyncedEWayBillList();

        // optimization
        if(ewayBillArrayList.size() == 0) {
    ...
    private String createOneJSONObject(EwayBill ewayBill) {
        try {
            JSONObject exportJson = new JSONObject();
            JSONArray jsonArray = new JSONArray();

            Gson gson = new Gson();
            String jsonString = gson.toJson(ewayBill);
            JSONObject jsonObject = new JSONObject(jsonString);
            jsonArray.put(jsonObject);

            Log.i("EWayBills", jsonArray.toString());
            exportJson.put("EWayBills", jsonArray);
        ...
        protected String doInBackground(Object... params) {
            ...
            try {
                for (EwayBill ewayBill : ewayBillArrayList) {
                    jsonString = createOneJSONObject(ewayBill);
                    ...
                    if(message.equals(MyFunctions.SUCCESS_MESSAGE)) {
                        DatabaseHandler.getInstance().singleEWayBillSyncSuccessful(ewayBill.getID());
                    } else {
                        break;
                    }
                }
    ...
    public JSONObject sendDataToServer(String url, String jsonString){
        ...
        try {
            ...
            url = Uri.parse(URL.GSTSendEWayBillInfoURL).buildUpon()
                    .appendQueryParameter("BusinessID", businessID)
                    .appendQueryParameter("EWayBills", jsonString).toString();

            urlHit = Uri.parse(URL.GSTSendEWayBillInfoURL).buildUpon()
